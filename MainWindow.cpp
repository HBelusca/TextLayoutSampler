...
void MainWindow::InitializeDrawableObjectsListView()
{
    ...
    for (auto const& attribute : DrawableObject::attributeList)
    {
        ...
        lc.cx = ((attribute.IsTypeArray() || attribute.id == 0) ? 120 : 80) * dpi / DPI_100;
...
{
	int dpi = GetDPI(hwnd_);
    const static ListViewColumnInfo columnInfo[] = {
        { 0, 120 * dpi / DPI_100, u"Attribute" },
        { 1, 400 * dpi / DPI_100, u"Value" },
    };
    ...
    case IddMainWindow:
        {
            ...
            windowPositionDrawableObjectsList.ClampRect({0x7FFF, 340 * dpi / DPI_100});
            windowPositionAttributesList.ClampRect({340 * dpi / DPI_100, 340 * dpi / DPI_100});
            windowPositionAttributeValuesList.ClampRect({340 * dpi / DPI_100, 340 * dpi / DPI_100});
            windowPositionEditText.ClampRect({0x7FFF, 160 * dpi / DPI_100});
            WindowPosition::ReflowGrid(windowPositions, uint32_t(countof(windowPositions)), clientRect, spacing, 0, PositionOptionsNone);
            ...
            windowPositionAttributeValuesEdit.ClampRect({0x7FFF, (dpi * 18) * ((GetWindowStyle(windowPositionAttributeValuesEdit.hwnd) & ES_MULTILINE) ? 3 : 1) / DPI_100});
            WindowPosition::ReflowGrid(&windowPositionAttributeValuesEdit, 3, attributeValuesRect, /*spacing*/0, 0, PositionOptionsFlowVertical | PositionOptionsUnwrapped);
